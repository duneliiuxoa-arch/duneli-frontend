...
export function MeetingPage({ discussion, currentTheme, userRole, userName, onLeave }: MeetingPageProps) {
  ...
  const [mobileTab, setMobileTab] = useState<'ideas' | 'queue' | 'people'>('ideas');
  const [showMobilePanel, setShowMobilePanel] = useState(false);

  return (
    <div className={`h-screen ${theme.textColor} flex flex-col overflow-hidden`} style={{ background: theme.background, fontFamily: 'var(--font-body)' }}>

      {/* ── Top Bar ── */}
      <div className={`${theme.cardStyle} px-3 sm:px-6 py-3 border-b ${borderCls} flex-shrink-0`}>
        <div className="flex items-center justify-between gap-2 w-full">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="flex items-center gap-1.5 bg-red-500 text-white px-2.5 py-1.5 rounded-full font-medium flex-shrink-0 text-xs">
          ...
          <div className={`flex items-center gap-3 ${theme.textColor} opacity-70 flex-shrink-0`}>
      ...
      {/* ── Speaking timer banner (mobile) ── */}
      {isSpeaking && speakerTimeLeft !== null && (
        <div className={`flex-shrink-0 flex items-center justify-between px-4 py-2 ${speakerTimeLeft <= 30 ? 'bg-red-500' : 'bg-indigo-500'} text-white text-sm`}>
          <div className="flex items-center gap-2 font-bold">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-white" />
            </span>
            Speaking — {formatTime(speakerTimeLeft)}
          </div>
          <button onClick={endSpeaking} className="text-xs underline opacity-80">Done</button>
        </div>
      )}

      {/* ── Your Turn banner (mobile) ── */}
      {speakingQueue.length > 0 && speakingQueue[0].userId === (currentUserId || userIdRef.current) && !isSpeaking && (
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-2 bg-green-500 text-white text-sm">
          <motion.button onClick={startSpeaking}
            animate={{ scale: [1, 1.04, 1] }} transition={{ repeat: Infinity, duration: 1.2 }}
            className="flex items-center gap-2 font-bold">
            <Mic className="w-4 h-4" /> Your Turn — Speak Now!
          </motion.button>
          {skipTimer !== null && (
            <span className={`text-xs font-bold ${skipTimer <= 3 ? 'text-red-200' : 'text-white/70'}`}>
              Skip in {skipTimer}s
            </span>
          )}
        </div>
      ...
      {/* ── Error banner ── */}
      {agoraError && (
        <div className="flex-shrink-0 mx-3 mt-2 px-3 py-2 rounded-xl bg-red-500/15 border border-red-500/30 text-red-400 text-xs">
          ⚠️ Audio: {agoraError}
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden w-full min-h-0">
        ...
        <div className={`w-64 xl:w-72 flex-shrink-0 ${theme.cardStyle} p-4 overflow-y-auto border-r ${borderCls} hidden md:flex md:flex-col gap-4`}>

          {/* Your Role */}
          <div>
            <h3 className={`text-xs uppercase tracking-wider opacity-60 mb-2 ${theme.textColor}`}>Your Role</h3>
            <div className={`${theme.buttonClass} rounded-2xl px-3 py-2.5 flex items-center gap-2`}>
              ...
              <span className="font-medium capitalize text-sm">{userRole}</span>
            </div>
          </div>

          {/* Speaker timer */}
          {speakerTimeLeft !== null && (
            <div className={`px-3 py-2.5 rounded-2xl flex items-center gap-2 ${speakerTimeLeft <= 30 ? 'bg-red-500 text-white' : theme.buttonClass}`}>
              <Clock className="w-4 h-4" />
              <span className="font-mono font-bold text-sm">{formatTime(speakerTimeLeft)}</span>
              <span className="text-xs opacity-80">left</span>
              <button onClick={endSpeaking} className="ml-auto text-xs underline opacity-70">Done</button>
            </div>
          )}
          ...
          {speakingQueue.length > 0 && (
            <div>
              <h3 className={`text-xs uppercase tracking-wider opacity-60 mb-2 ${theme.textColor}`}>
              ...
              <div className="space-y-1.5">
                {speakingQueue.map((q, i) => {
                  ...
                  return (
                    <div key={q.userId} className={`rounded-xl px-3 py-2.5 flex items-center gap-2 ${
                      isMe ? 'bg-indigo-500/20 border border-indigo-500/40'
                        : i === 0 ? isDark ? 'bg-green-500/15 border border-green-500/30' : 'bg-green-50 border border-green-200'
                        : theme.cardStyle
                    }`}>
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${i === 0 ? 'bg-green-500 text-white' : theme.buttonClass}`}>{i + 1}</div>
                      <span className={`flex-1 text-xs font-medium truncate ${isMe ? 'text-indigo-400' : ''}`}>{isMe ? 'You' : q.name}</span>
                      {i === 0 && <span className="text-[9px] text-green-400 font-bold">NEXT</span>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            <div className="space-y-1.5">
              {uniqueParticipants.map(p => (
                <div key={p.id} className={`rounded-xl px-3 py-2.5 flex items-center gap-2 relative ${p.isSpeaking ? theme.buttonClass : `${theme.cardStyle} opacity-80`}`}>
                  {(p as any).handRaised && <span className="absolute -top-1 -right-1 text-xs">✋</span>}
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${p.isSpeaking ? 'bg-white/20' : 'bg-white/10'}`}>
                    {p.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-xs truncate">{p.name}</p>
                    <p className="text-[10px] opacity-60 capitalize">{p.role}</p>
                  </div>
                  {p.isSpeaking && (
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          {/* Mobile Tab Bar */}
          <div className={`md:hidden flex border-b ${borderCls} flex-shrink-0`}>
            {(['ideas', 'queue', 'people'] as const).map(tab => (
              <button key={tab} onClick={() => setMobileTab(tab)}
                className={`flex-1 py-2.5 text-xs font-semibold capitalize transition-all ${
                  mobileTab === tab
                    ...
                    : `opacity-50 ${theme.textColor}`
                }`}>
                {tab === 'ideas' ? '💡 Ideas' : tab === 'queue' ? `✋ Queue${speakingQueue.length > 0 ? ` (${speakingQueue.length})` : ''}` : `👥 People (${uniqueParticipants.length})`}
              </button>
            ))}
          </div>

          {/* Current Speaker Stage — desktop only */}
          <div className={`hidden md:block p-4 border-b ${borderCls} flex-shrink-0`}>
            {currentSpeaker ? (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                className={`${theme.cardStyle} rounded-2xl p-4 flex items-center gap-4`}>
                <div className={`w-12 h-12 rounded-full ${theme.buttonClass} flex items-center justify-center text-xl font-bold flex-shrink-0`}>
                  {currentSpeaker.name.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className={`font-bold truncate ${theme.textColor}`}>{currentSpeaker.name}</h2>
                  <p className={`text-xs opacity-60 capitalize`}>{currentSpeaker.role} • Speaking</p>
                  {liveTranscript && <p className="text-xs italic opacity-70 mt-1 truncate">🎙️ {liveTranscript}</p>}
                </div>
              ...
              <div className={`${theme.cardStyle} rounded-2xl p-3 flex items-center gap-3 opacity-40`}>
                <Volume2 className="w-5 h-5" />
                <p className="text-sm">No one is speaking yet</p>
              </div>
            )}
          </div>

          {/* ── Ideas Tab (mobile) / Ideas Stream (desktop) ── */}
          <div className={`flex-1 flex flex-col overflow-hidden min-h-0 ${mobileTab !== 'ideas' ? 'hidden md:flex' : 'flex'}`}>
            {/* Header */}
            <div className={`px-4 pt-3 pb-2 flex items-center justify-between border-b ${borderCls} flex-shrink-0`}>
              <div className="flex items-center gap-2">
                <span className={`text-xs uppercase tracking-wider opacity-60 font-semibold ${theme.textColor}`}>Ideas Shared</span>
                {ideas.length > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${isDark ? 'bg-white/10 text-white/50' : 'bg-gray-100 text-gray-500'}`}>{ideas.length}</span>
                )}
              </div>
              {canShareIdea && ideaCooldown > 0 && (
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-mono font-semibold ${
                  ideaCooldown <= 30 ? 'bg-green-500/15 text-green-400' : isDark ? 'bg-yellow-500/12 text-yellow-400' : 'bg-yellow-50 text-yellow-600'
                }`}>
                  <Clock className="w-3 h-3" />
                  {Math.floor(ideaCooldown / 60)}:{(ideaCooldown % 60).toString().padStart(2, '0')}
                </div>
              )}
              {!canShareIdea && <span className={`text-[10px] px-2 py-0.5 rounded-full ${isDark ? 'bg-white/6 text-white/40' : 'bg-gray-100 text-gray-400'}`}>👁 Read-only</span>}
            </div>

            {/* Ideas list */}
            <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3 min-h-0">
              {ideas.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-2 py-10 opacity-40">
                  <Volume2 className="w-8 h-8" />
                  <p className="text-sm font-medium">No ideas shared yet</p>
                  <p className="text-xs">The debate will unfold here.</p>
                </div>
              ) : (
                <AnimatePresence initial={false}>
                  {ideas.map(idea => {
                    ...
                    const timeLabel = secondsAgo < 60 ? `${secondsAgo}s` : `${Math.floor(secondsAgo / 60)}m`;
                    return (
                      <motion.div key={idea.id}
                        ...
                        <div className={`flex items-center gap-1.5 mb-1 ${isOwn ? 'flex-row-reverse' : ''}`}>
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${isDark ? 'bg-indigo-500/30 text-indigo-300' : 'bg-indigo-100 text-indigo-600'}`}>
                            {idea.speakerName.charAt(0).toUpperCase()}
                          </div>
                          <span className={`text-[10px] font-semibold ${isDark ? 'text-white/50' : 'text-gray-500'}`}>{isOwn ? 'You' : idea.speakerName}</span>
                          <span className={`text-[9px] ${isDark ? 'text-white/25' : 'text-gray-300'}`}>{timeLabel}</span>
                        </div>
                        <div className={`max-w-[88%] sm:max-w-[80%] px-3 py-2.5 rounded-2xl text-sm leading-relaxed ${
                          isOwn ? 'bg-indigo-500 text-white rounded-tr-md'
                            : isDark ? 'bg-white/10 text-white rounded-tl-md border border-white/8'
                            : 'bg-white text-gray-800 rounded-tl-md border border-gray-200'
                        }`}>{idea.content}</div>
                        <div className={`flex items-center gap-1 mt-1.5 ${isOwn ? 'flex-row-reverse' : ''}`}>
                          <button onClick={() => handleIdeaReaction(idea.id, 'agree')}
                            className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-all active:scale-95 ${
                              idea.hasUserAgreed ? 'bg-green-500 text-white' : isDark ? 'bg-white/8 hover:bg-white/14 text-white/50' : 'bg-gray-100 text-gray-500'
                            }`}>
                          ...
                          <button onClick={() => handleIdeaReaction(idea.id, 'disagree')}
                            className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-all active:scale-95 ${
                              idea.hasUserDisagreed ? 'bg-red-500 text-white' : isDark ? 'bg-white/8 hover:bg-white/14 text-white/50' : 'bg-gray-100 text-gray-500'
                            }`}>
            ...
            {/* Idea Input */}
            {canShareIdea && (
              <div className={`px-3 pb-3 pt-2 border-t ${borderCls} flex-shrink-0`}>
                <AnimatePresence mode="wait">
                  {ideaCooldown > 0 ? (
                    <motion.div key="cooldown" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-2xl ${isDark ? 'bg-white/5 border border-white/10' : 'bg-gray-50 border border-gray-200'}`}>
                      <Clock className={`w-4 h-4 flex-shrink-0 ${ideaCooldown <= 30 ? 'text-green-400' : 'opacity-40'}`} />
                      <p className={`text-xs flex-1 ${isDark ? 'text-white/40' : 'text-gray-400'}`}>
                        Next idea in {Math.floor(ideaCooldown / 60)}:{(ideaCooldown % 60).toString().padStart(2, '0')}
                      </p>
                    </motion.div>
                  ) : (
                    <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      className={`flex items-end gap-2 px-3 py-2 rounded-2xl border ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
                      <textarea value={ideaInput} onChange={e => setIdeaInput(e.target.value)} onKeyDown={handleIdeaKey}
                        placeholder="Share an idea..." rows={1}
                        className={`flex-1 bg-transparent outline-none resize-none text-sm leading-snug ${theme.textColor} placeholder-current opacity-100 placeholder:opacity-40 max-h-20`} />
                      <button onClick={submitIdea} disabled={!ideaInput.trim()}
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${ideaInput.trim() ? 'bg-indigo-500 text-white hover:bg-indigo-600' : isDark ? 'bg-white/10 text-white/20' : 'bg-gray-100 text-gray-300'}`}>
            ...
            {!canShareIdea && (
              <div className={`px-3 pb-3 pt-2 border-t ${borderCls} flex-shrink-0`}>
                <p className={`text-xs text-center opacity-40 ${theme.textColor}`}>Idea sharing is for speakers &amp; debaters only</p>
              </div>
            )}
          </div>

          {/* ── Queue Tab (mobile only) ── */}
          <div className={`md:hidden flex-1 overflow-y-auto p-4 ${mobileTab !== 'queue' ? 'hidden' : 'block'}`}>
            {speakingQueue.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 opacity-40 gap-2">
                <Hand className="w-8 h-8" />
                <p className="text-sm">No one in queue</p>
                <p className="text-xs">Raise hand to join</p>
              </div>
            ) : (
              <div className="space-y-2">
                <h3 className={`text-xs uppercase tracking-wider opacity-60 mb-3 ${theme.textColor}`}>Speaking Queue ({speakingQueue.length})</h3>
                {speakingQueue.map((q, i) => {
                  const isMe = q.userId === (currentUserId || userIdRef.current);
                  return (
                    <div key={q.userId} className={`rounded-2xl px-4 py-3 flex items-center gap-3 ${
                      isMe ? 'bg-indigo-500/20 border border-indigo-500/40'
                        : i === 0 ? isDark ? 'bg-green-500/15 border border-green-500/30' : 'bg-green-50 border border-green-200'
                        : theme.cardStyle
                    }`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${i === 0 ? 'bg-green-500 text-white' : theme.buttonClass}`}>{i + 1}</div>
                      <span className={`flex-1 font-medium ${isMe ? 'text-indigo-400' : ''}`}>{isMe ? 'You' : q.name}</span>
                      {i === 0 && <span className="text-xs text-green-400 font-bold">NEXT</span>}
                    </div>
                  );
                })}
              </div>
            ...
            {/* Participants on mobile queue tab */}
            <div className="mt-6">
              <h3 className={`text-xs uppercase tracking-wider opacity-60 mb-3 ${theme.textColor}`}>Participants ({uniqueParticipants.length})</h3>
              <div className="space-y-2">
                {uniqueParticipants.map(p => (
                  <div key={p.id} className={`rounded-2xl px-4 py-3 flex items-center gap-3 ${p.isSpeaking ? theme.buttonClass : theme.cardStyle}`}>
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold ${p.isSpeaking ? 'bg-white/20' : 'bg-white/10'}`}>
                      {p.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    {(p as any).handRaised && <span className="text-sm">✋</span>}
                    {p.isSpeaking && (
          ...
          {/* ── People Tab (mobile only) ── */}
          <div className={`md:hidden flex-1 overflow-y-auto p-4 ${mobileTab !== 'people' ? 'hidden' : 'block'}`}>
            <h3 className={`text-xs uppercase tracking-wider opacity-60 mb-3 ${theme.textColor}`}>In Room ({uniqueParticipants.length})</h3>
            <div className="space-y-2">
              {uniqueParticipants.map(p => (
                <div key={p.id} className={`rounded-2xl px-4 py-3 flex items-center gap-3 ${p.isSpeaking ? theme.buttonClass : theme.cardStyle}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold ${p.isSpeaking ? 'bg-white/20' : 'bg-white/10'}`}>
                    {p.name.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">{p.name}</p>
                    <p className="text-xs opacity-60 capitalize">{p.role}{p.isSpeaking ? ' • Speaking' : ''}</p>
                  </div>
                  {(p as any).handRaised && <span>✋</span>}
                  {p.isSpeaking && (
          ...
          {/* ── Bottom Controls ── */}
          <div className={`${theme.cardStyle} px-3 sm:px-4 py-3 border-t ${borderCls} flex-shrink-0`}>
            <div className="flex items-center justify-between gap-2 w-full">
              <div className="flex items-center gap-2">
                ...
                {(userRole === 'listener' || userRole === 'speaker') && !isSpeaking && (
                  <button onClick={handleHandRaise}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-2xl font-medium transition-all text-sm ${
                    ...
                    <span className="sm:hidden">{handRaised ? '✋' : '🖐'}</span>
                  </button>
                )}

                {/* Speaking badge (mobile compact) */}
                {isSpeaking && (
                  <button onClick={endSpeaking}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-red-500/20 text-red-400 text-sm font-medium">
                    <span className="relative flex h-2 w-2">
                ...
                {(userRole === 'debater' || userRole === 'speaker') && (
                  <button onClick={handleMicToggle}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-2xl font-medium transition-all text-sm ${!micMuted ? 'bg-green-500 text-white' : `${theme.cardStyle} hover:bg-white/10`}`}>
                ...
                <button onClick={() => setChatOpen(o => !o)}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-2xl font-medium transition-all text-sm relative ${chatOpen ? theme.buttonClass : `${theme.cardStyle} hover:bg-white/10`}`}>
              ...
              <button onClick={onLeave}
                className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-red-500/20 hover:bg-red-500 text-red-400 hover:text-white font-medium transition-all text-sm">
        ...
        <AnimatePresence>
          {chatOpen && (
            <motion.div key="chat"
              ...
              className={`fixed inset-0 z-50 md:relative md:inset-auto md:w-80 md:flex-shrink-0 flex flex-col ${theme.cardStyle} border-l ${borderCls}`}>
              <div className={`px-4 py-3 border-b ${borderCls} flex items-center justify-between flex-shrink-0`}>
                <h3 className={`font-semibold text-sm ${theme.textColor}`}>Room Chat</h3>
                <button onClick={() => setChatOpen(false)} className="p-1.5 rounded-xl hover:bg-white/10">
                  <X className="w-4 h-4" />
                </button>
              </div>
                ...
                {chatMsgs.length === 0 ? (
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                      {!isOwn && <span className={`text-[10px] mb-0.5 opacity-50 ${theme.textColor}`}>{msg.user.name || msg.user.anonymousId}</span>}
                      <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm ${isOwn ? msgBubbleOwn + ' rounded-tr-md' : msgBubbleOther + ' rounded-tl-md'} ${msg.pending ? 'opacity-60' : ''}`}>
                        {msg.message}
                      </div>
                    </div>
                  );
              ...
              <div className={`px-3 py-3 border-t ${borderCls} flex items-end gap-2 flex-shrink-0`}>
                <textarea value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={handleChatKey}
                  placeholder="Message..." rows={1}
                  className={`flex-1 bg-transparent outline-none resize-none text-sm ${theme.textColor} placeholder-current opacity-100 placeholder:opacity-40 max-h-20`} />
                <button onClick={sendMessage} disabled={!chatInput.trim() || chatSending}
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${chatInput.trim() ? 'bg-indigo-500 text-white' : isDark ? 'bg-white/10 text-white/20' : 'bg-gray-100 text-gray-300'}`}>
                  <Send className="w-3.5 h-3.5" />
                </button>
              </div>
            </motion.div>
